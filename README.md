...
<a href="https://drsiyarb.github.io/hox-morphospace/">
  <img src="https://img.shields.io/badge/%E2%96%B6%20LAUNCH%20THE%20HOX%20MORPHOSPACE%20EXPLORER-2ea44f?style=for-the-badge&logoColor=white" alt="Launch the Hox Morphospace Explorer" height="48">
</a>

#### → [**Open the interactive 3D explorer**](https://drsiyarb.github.io/hox-morphospace/) ←
...
The **Hox Morphospace Explorer** is a self-contained HTML file generated by `scripts/generate_morphospace_hoxaxes.py`. It provides an interactive interface for navigating the morphospace in real time.
...
```
hox-morphospace/
├── index.html                      # Landing page → redirects to the explorer
├── hox_morphospace_explorer.html   # The interactive 3D explorer (self-contained)
├── README.md
├── LICENSE
├── scripts/                        # Generation & figure scripts
│   ├── generate_morphospace_hoxaxes.py   # Main: builds the explorer
│   ├── generate_morphospace_pure.py
│   ├── generate_plane.py
│   └── fig1–4_*.py                       # Publication figure scripts
├── figures/                        # Publication figures (300 DPI PNGs)
├── data/                           # Precomputed coordinates
│   └── hox_morphospace_boxcentered.csv
└── blender/                        # Blender add-on + export/import scripts
```
...
1. **Build the interactive explorer** from the precomputed coordinates (`data/hox_morphospace_boxcentered.csv`) and the Hox expression in the h5ad:
   ```bash
   python scripts/generate_morphospace_hoxaxes.py
   ```
   This reads `data/hox_morphospace_boxcentered.csv` and writes the interactive HTML explorer.

2. **Generate publication figures**:
   ```bash
   python scripts/fig1_pca_to_planes.py
   python scripts/fig2_hox_morphospace.py
   python scripts/fig3_geometry_celltypes.py
   python scripts/fig4_expression_landscapes.py
   ```
...
| Precomputed coordinates | `data/hox_morphospace_boxcentered.csv` |
